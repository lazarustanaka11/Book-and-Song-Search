...
class Google(object):
    ...
    def book_search(self, query):
        ...
        results, book_result_status, book_time_lapse = self.init_calls.init_request(
            self.book_base_url, self.book_payload
        )
        ...
        if len(results) > 0:
            for x in range(len(results)):
                ...
                author = (
                    volume_info["authors"] if "authors" in volume_info.keys() else []
                )  # if no author return empty list
                title = (
                    volume_info["title"] if "title" in volume_info.keys() else []
